...
def main(args):
    path = './Predictions/Helpdesk-Best/DLS'
    model_types = []
    logs = []
    ...
    #     distributions, _ = data_preprocessing.create_distributions(logs_dir, log_name='BPI_Challenge_2013_incidents.xes.gz')
    ...
    for file_name in os.listdir(path):
        if file_name.startswith('suffix_'):
    ...
    # table_rows = []
    # for file_name in os.listdir(path):
    #     if file_name.startswith('suffix_generation_result'):
    ...
    # table_rows = []
    # for file_name in os.listdir(path):
    #     if file_name.startswith('suffix_generation_result_'):
    #         result_json_file_name = os.path.join(path, file_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--recalculate_counts', help='recalculate_counts', default=False, type=bool)
