...
def generate(datetime, model_type, args, combi=[]):
    ...
    for checkpoint_file in checkpoint_files:
        ...
        with open(os.path.join(pred_path, f'suffix_evaluation_result_{combi[0]}_{combi[-1]}[{number}].json'), 'w') as fp:
...
def evaluate_generation(datetime, model_type, combi=[]):
    ...
    for checkpoint in checkpoint_files:
        ...
        with open(os.path.join(pred_path, f'suffix_evaluation_result_dls_mae_{combi[0]}_{combi[-1]}[{number}].json'), 'w') as fp:
    ...
    # Merge evaluation results (per model type):
        ...
        with open(os.path.join(pred_path, f'suffix_evaluation_result_dls_mae_{combi[0]}_{combi[-1]}[{number}].json')) as f_in:
