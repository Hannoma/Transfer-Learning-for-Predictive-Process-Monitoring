...
def iterate_over_traces(log_with_traces,
    ...
    for mini_batch in iter(data_loader):
        ...
        # Loss is calculated only for the masked positions:
            ...
            categorical_loss = categorical_criterion(prediction[0][:, model.mlm.masked_indexes, :].transpose(2, 1),
        ...
        if len(prediction) > 1:
            ...
            if model.architecture == 'GPT':
                ...
                regression_loss = regression_criterion(prediction[1][:, model.mlm.masked_indexes, :],
...
def main(args, dt_object, pre_train=True, combi=[], layers=[]):
    ...
    if not pre_train:
        ...
        path = os.path.join(RESULTS_DIR, str(args.architecture), str(processed_log['id']))
        ...
        nb_special_tokens = 4
        attributes_meta = {
            0: {'nb_special_tokens': nb_special_tokens, 'vocabulary_size': augmented_log['vocabulary_size']},
    ...
    path = os.path.join(RESULTS_DIR, str(args.architecture), str(new_processed_log['id']))
    if not os.path.exists(path): os.makedirs(path)
    ...
    nb_special_tokens = 4
    attributes_meta = {
        0: {'nb_special_tokens': nb_special_tokens, 'vocabulary_size': new_augmented_log['vocabulary_size']},
        1: {'min_value': 0.0, 'max_value': 1.0}}
    ...
    for layer in layers:
        ...
        for e in range(args.nb_epoch):
            ...
            if len(validation_loss) > 1:
                ...
                if args.architecture == 'BERT':
                    total_validation_loss_fix_masks = validation_loss_fix_masks[0] + args.lagrange_a * \
                                                      validation_loss_fix_masks[1]
                else:
                    # an arbritary value:
            ...
            if args.early_stopping:
                ...
                if args.architecture == 'BERT':
                    if len(total_validation_losses_fix_masks) > early_stopping_var:
                        if np.all(np.array(total_validation_losses_fix_masks)[-(early_stopping_var - 1):] >
                                  np.array(total_validation_losses_fix_masks)[-early_stopping_var]):
            ...
            new_path = os.path.join(RESULTS_DIR, 'GPT', combi[-1])
